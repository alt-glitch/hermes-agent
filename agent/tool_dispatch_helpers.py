...
_PARALLEL_SAFE_TOOLS = frozenset({
    # tool_gateway.names.CONNECTOR_BATCH_SENTINEL: a tool_call calls[] batch.
    # Network-bound, no local mutable state, and each dispatch carries its
    # own idempotency key — batches may run alongside other admitted calls.
    "connectors__execute",
    "ha_get_state",
    "ha_list_entities",
    "ha_list_services",
...
def _peel_bridge_call(tool_name: str, function_args: dict) -> tuple[str, dict]:
    ...
    try:
        from tools.tool_search import (
            CONNECTOR_BATCH_SENTINEL, TOOL_CALL_NAME, is_connector_name, resolve_underlying_call)
        if tool_name == TOOL_CALL_NAME:
            ...
            if err is None and underlying:
                if underlying == CONNECTOR_BATCH_SENTINEL:
                    ...
                    # Only a pure remote batch is parallel-safe. Mixed/local
                    # batches remain barriers because each local entry still
                    # needs path/MCP admission in order.
                    if entries and all(isinstance(entry, dict) and is_connector_name(entry.get("name"))
