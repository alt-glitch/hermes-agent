...
def _last_visible_reasoning_for_turn(messages, final_response) -> Optional[Any]:
    """Latest current-turn reasoning that is not a duplicate of the visible answer.

    Compare against both transformed output and provider-authored assistant content;
    persisted reasoning metadata remains untouched for replay continuity.
    """
    ...
    provider_answer = next((msg.get("content") for msg in current_turn
                            if msg.get("role") == "assistant"
                            and isinstance(msg.get("content"), str)
                            and msg.get("content").strip()), None)
