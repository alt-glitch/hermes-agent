...
def probe(surface, scenario, directory):
    ...
    def tui_submit(_rid, _sid, _session, text, **kwargs):
        accepted = submit(text)
        if accepted and (callback := kwargs.get("history_commit_callback")) is not None:
            callback(server._HistoryCommitOutcome(True, True, True))
        return accepted
