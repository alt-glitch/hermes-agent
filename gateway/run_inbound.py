...
class GatewayInboundMixin:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        _loop_claim_id = str(
            (getattr(event, "metadata", None) or {}).get("hermes_loop_claim_id") or ""
        )
        try:
            try:
            ...
            if _loop_claim_id:
                try:
                    entry = self.session_store.lookup_by_session_key(_quick_key)
                    sid = getattr(entry, "session_id", None) or ""
                    if sid:
                        from hermes_cli.loops import LoopManager
                        mgr = LoopManager(session_id=sid)
                        if mgr.state is not None and mgr.state.awaiting_response:
                            mgr.abandon_tick(_loop_claim_id)
                except Exception:
