...
class GatewayGoalsMixin:
    ...
    async def _post_turn_loop_completion(
        ...
        *,
        ...
        if state is None or not state.awaiting_response:
            return
        # Only the wakeup turn that carries the persisted claim may complete
        # it. An unrelated turn for the same session must leave it untouched.
        if not claim_id:
            return
    ...
    async def _fire_due_loop_wakeups_once(
        ...
        *,
        ...
        # Warm the shared goals/loops SessionDB cache inside the active
        # profile scope before the first persisted-loop read.
        ...
        for sid, state in active_loops:
            ...
            try:
                entry = await self._run_in_executor_with_context(
                    self.session_store.lookup_by_session_id, sid
                )
            except Exception:
                entry = None
            ...
            if session_key:
                try:
                    route_entry = await self._run_in_executor_with_context(
                        self.session_store.lookup_by_session_key, session_key
                    )
                except Exception:
                    route_entry = None
                if route_entry is not None and getattr(route_entry, "resume_pending", False):
                    continue
            if await self._run_in_executor_with_context(goal_blocks_loop_tick, sid):
                continue

            mgr = await self._run_in_executor_with_context(
                lambda: LoopManager(session_id=sid)
            )
            if state.awaiting_response and not await self._run_in_executor_with_context(
                mgr.recover_stale_tick, scan_now
            ):
                continue
            if not await self._run_in_executor_with_context(mgr.is_due, scan_now):
                continue
            wakeup = await self._run_in_executor_with_context(mgr.fire_tick)
            if not wakeup:
                continue
