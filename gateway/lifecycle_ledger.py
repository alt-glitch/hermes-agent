...
def _integrity_check_lease_renewal(started_at: float):
    """Return a best-effort startup-watchdog renewal callback."""
    from hermes_startup_watchdog import report_startup_progress

    report_startup_progress(_INTEGRITY_CHECK_LEASE_S, phase=_INTEGRITY_CHECK_LEASE_PHASE)
    last_renew = started_at

    def _renew(now: float) -> None:
        nonlocal last_renew
...
def check_state_db_integrity(
    ...
    *,
    ...
    or the first integrity/check failure. A timeout is deliberately inconclusive.
    The SQLite progress callback renews the startup-watchdog lease while work
    progresses, but aborts the PRAGMA at this helper's stricter bounded deadline.
    """
    ...
    renew_lease = _integrity_check_lease_renewal(started_at)
    ...
    try:
        with closing(sqlite3.connect(str(path))) as conn:
            conn.set_progress_handler(_on_progress, STATE_DB_INTEGRITY_PROGRESS_STEPS)
            try:
