...
- **Inactivity watchdog, not a fixed wall-clock cutoff.** A job's
  `inactivity_timeout_seconds` takes precedence; otherwise `HERMES_CRON_TIMEOUT` applies, with a
  600-second default and `0` meaning unlimited. A stalled session is hard-interrupted so it cannot
  monopolise the scheduler, while a long-but-active job is never cut off. Attached scripts (pre-run
  or `no_agent`) are bounded separately by the script timeout (`_DEFAULT_SCRIPT_TIMEOUT`, 3600s).
  Long-running maintainers also need their own bounded lease and durable execution/claim
  heartbeats; activity is not permission to bypass ownership checks.
