...
_running_run_claim_tokens: dict[tuple, str] = {}
...
_gated_dispatches: dict[tuple, _GatedDispatch] = {}
...
def try_register_running_job(
    ...
    with retirement.work() as admitted, _running_lock:
        ...
        if run_claim_token:
            _running_run_claim_tokens[key] = run_claim_token
        if gated_dispatch is not None:
            _gated_dispatches[key] = gated_dispatch
...
def release_running_job(job_id: str, home: Optional[Union[Path, str]] = None) -> None:
    ...
    with _running_lock:
        ...
        _running_run_claim_tokens.pop(key, None)
        _gated_dispatches.pop(key, None)
        _running_worker_pids.pop(key, None)
        # Narrow compatibility for callers/tests that still seed bare ids directly. Production
        # registrations always use the profile-qualified key above.
...
def sweep_stale_inflight(due_jobs: Optional[list] = None) -> list:
    ...
    with _running_lock:
        for key in [k for k in _running_job_ids if k[0] == _local_home]:
            ...
            _running_futures.pop(key, None)
            _running_run_claim_tokens.pop(key, None)
            pending = _gated_dispatches.pop(key, None)
            if pending is not None:
                pending.cancelled.set()
                cancelled_dispatches[key] = pending
            _forced_release_count += 1
            stale.append((job_id, age, allowance, fut, reason))

    for key, pending in cancelled_dispatches.items():
        _finish_cancelled_dispatch(key[1], pending, "stale in-flight registration")
...
def mark_running_jobs_interrupted(
    ...
    with _running_lock:
        ...
        cancelled_dispatches = {
            key: pending for key, pending in _gated_dispatches.items()
            if key not in restart_safe_waiters
        } if only_owners is None else {}
        ...
        if only_owners is None:
            ...
            active_fires.extend(
                ...
                for key in (
                    _running_job_ids - registered_keys - restart_safe_waiters - cancelled_dispatches.keys()
                )
            )
    ...
    for _token, key, fire_owner, profile_home in active_fires:
        job_id = key[1]
        if not fire_owner:
            run_token = run_claim_tokens.get(key)
            if run_token:
                try:
...
def _run_agent_with_watchdog(
    ...
    watchdog: a per-job value wins; otherwise HERMES_CRON_TIMEOUT overrides the
    600s global default. A zero value means unlimited."""
        ...
        try:
            ...
            with _running_lock:
                if dispatch_cancelled.is_set():
                    return False
                _gated_dispatches.pop(_inflight_key(job_id, home), None)
