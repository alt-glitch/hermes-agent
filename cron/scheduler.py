...
def try_register_running_job(
    job_id: str, *, run_claim_token: str | None = None
) -> bool:
    ...
    with _running_lock:
        ...
        if run_claim_token:
            _running_run_claim_tokens[job_id] = str(run_claim_token)
...
def mark_running_jobs_interrupted(
    ...
    *,
    ...
    with _running_lock:
        claim_tokens = {
            job_id: _running_run_claim_tokens.get(job_id)
    ...
    for _token, job_id, fire_owner, profile_home in active_fires:
        if not fire_owner:
            run_claim_token = claim_tokens.get(job_id)
            if run_claim_token is not None:
                try:
                    with use_cron_store(profile_home):
                        if mark_job_run(
                            ...
                            False,
                            reason,
                            expected_run_claim_token=run_claim_token,
                        ):
...
def _deliver_result(
    ...
    for target in targets:
        ...
        resolve_target_adapters = getattr(adapters, "for_target", None)
        if callable(resolve_target_adapters):
            ...
            target_adapters = resolve_target_adapters(platform, target)
                ...
                try:
                    ...
                    try:
                        ...
                        def _run_standalone_send():
                            ...
                            # pool.submit() rejects during interpreter
                            # shutdown, no unowned coroutine is created.
                            return asyncio.run(
...
class SharedRouteAdapters:
    ...
    def __init__(self, primary_adapters, routes) -> None:
        # Normalize enum/string keys at the boundary.  Long-lived gateway
        # processes and test/plugin reloads can otherwise leave equivalent
        # Platform members with different class identities.
        self._primary = {
            str(getattr(platform, "value", platform)).lower(): adapter
            for platform, adapter in dict(primary_adapters or {}).items()
        }
    ...
    def get(self, platform, target=None, default=None):
        ...
        adapter = self._primary.get(platform_key)
        if adapter is None:
            return default
        if isinstance(target, dict):
            ...
            # DeliveryTarget is canonical; keep dict compatibility for queued
            # legacy callers and focused fixtures.
            raw_chat_id = getattr(target, "chat_id", None)
            thread_id = getattr(target, "thread_id", None)
        chat_id = str(raw_chat_id or "") or None
        ...
        for route in self._routes:
            ...
            if route.matches(str(route.platform), chat_id=chat_id, thread_id=thread_id):
                return adapter
        return default
    def for_target(self, platform, target) -> dict:
        """Return the live-adapter map authorized for one exact target."""
        adapter = self.get(platform, target)
        return {platform: adapter} if adapter is not None else {}
...
def run_job(
    ...
    *,
    ...
    try:
        ...
        # override globally via HERMES_CRON_TIMEOUT or for this job via
        # inactivity_timeout_seconds.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _cron_inactivity_limit = resolve_cron_inactivity_timeout_seconds(job)
        _POLL_INTERVAL = 5.0
        ...
        try:
            ...
            if (
                ...
                and not (_is_oneshot and _run_claim_heartbeat_kwargs)
...
def _run_one_job_body(
    ...
    *,
    ...
    def _mark_this_dispatch(
        ...
        *,
        ...
        if expected_run_claim_token:
            kwargs["expected_run_claim_token"] = expected_run_claim_token
    ...
    try:
        ...
        marked = _mark_this_dispatch(
            success,
            error,
            delivery_error=delivery_error,
            status="blocked_config" if blocked_config else None,
        )
        if (fire_owner is not None or expected_run_claim_token) and not marked:
            _finish_execution_best_effort(
                execution_id,
                success=False,
                error=(
                    ...
                    if fire_owner is not None
                    else "Run claim ownership lost before terminal completion."
                ),
            )
            return True
        ...
        try:
            if not _consume_interrupted_flag(job["id"], execution_token):
                _mark_this_dispatch(
                    False,
                    _err_text,
                    delivery_error=(delivery_error if isinstance(e, Exception) else None),
                )
        ...
        # The execution ledger must reach a terminal state even when the jobs
        # store is unavailable; never let ledger I/O mask the original error.
        _finish_execution_best_effort(
            execution_id,
            success=False,
            error=_err_text,
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Successfully submitted recurring jobs are batch-advanced after their
        # execution rows exist but before their start gates open. The worker's
        # later claim_job_for_fire CAS independently revalidates ownership when
        # execution actually starts, so queued work cannot consume a claim TTL.
        ...
        def _process_job(job: dict) -> bool:
            ...
            try:
                ...
                _finish_execution_best_effort(
                    job["execution_id"],
                    success=False,
                    error=f"Fire claim failed: {claim_err}",
                )
                return False
            if not claimed:
                _finish_execution_best_effort(
        ...
        def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor):
            ...
            if not try_register_running_job(
                job_id, run_claim_token=run_claim_token
            ):
            ...
            def _run_and_release(
                ...
                try:
                    # Executor submission must succeed before the schedule is
                    # advanced, but no job side effect may begin until that
                    # advance is durable. The caller opens every gate only
                    # after its one batch write succeeds.
                    gate.wait()
                    if cancelled.is_set():
                        return False
