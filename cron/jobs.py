...
def _normalize_inactivity_timeout_seconds(value: Any) -> Optional[int]:
    """Normalize a per-job inactivity budget; ``None``/``inherit`` means global default."""
    if value is None:
        return None
    ...
    if isinstance(value, int):
        ...
        try:
            ...
            raise ValueError(
                "inactivity_timeout_seconds must be a non-negative integer or 'inherit'") from exc
...
def resolve_cron_inactivity_timeout_seconds(
    ...
    """Return the effective job inactivity limit; ``0`` disables the watchdog."""
    if job is not None and "inactivity_timeout_seconds" in job:
        try:
            ...
            logger.warning(
                "Invalid inactivity_timeout_seconds=%r for cron job %r; using inherited timeout",
                job.get("inactivity_timeout_seconds"), job.get("id") or job.get("name"))
        else:
            if configured is not None:
                return configured if configured > 0 else None
    # Profile-scoped under multiplex: the tick may run in the default profile's process.
...
def cron_store_transaction():
    """Hold the store lock across compare/mutate/verify/compensate workflows."""
    with _jobs_lock():
        yield
...
def create_job(
    ...
    for key, value in (
        ...
        ("inactivity_timeout_seconds", normalized_inactivity_timeout),
    ):
...
def rearm_oneshot(job_id: str, run_at: Any) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, _i, job):
        now = _hermes_now()
        if _claim_is_live(job.get("run_claim"), now, _oneshot_run_claim_ttl_seconds(job)):
...
def claim_job_for_fire(
    job_id: str, *, claim_ttl_seconds: int = FIRE_CLAIM_TTL_SECONDS,
    force: bool = False, return_job: bool = False,
    manual: bool = False,
    expected_run_claim_token: Optional[str] = None,
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if (
        ...
        and _claim_is_live(
            job.get("run_claim"), now, _oneshot_run_claim_ttl_seconds(job)
        )
    ):
        return False
