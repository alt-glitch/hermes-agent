    ...
    for job in jobs:
        ...
        try:
            ...
            if existing_claim and job.get("schedule", {}).get("kind") == "once":
                ...
                # Token-bearing claims can be safely reclaimed because later
                # completion is fenced by token. A legacy claim is reclaimable
                # only when it names this process, whose running registry is
                # authoritative; preserve unfenced foreign claims.
                stale_run_claim = bool(existing_claim.get("token")) or (
                    existing_claim.get("by") == _machine_id()
