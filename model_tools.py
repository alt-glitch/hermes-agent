...
def get_tool_definitions(enabled_toolsets: Optional[List[str]] = None, disabled_toolsets: Optional[List[str]] = None,
    ...
    if cached is None:
        ...
        if cache_key is None:
            return list(result)
        # A lazy registration or availability invalidation may race the compute.
        # Never publish a result under a key whose generation/verdicts are stale.
        if cache_key[3] != registry._generation or cache_key[-1] != registry.check_fn_verdict_snapshot():
            return list(result)
        with _tool_defs_cache_lock:
...
def handle_function_call(
    ...
    _execution_observed: Optional[List[bool]] = None,
) -> str:
