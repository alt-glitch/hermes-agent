...
// OpenTUI kitty input parses these into a single `return` key event with the
// matching modifier flag, so the composer newline keybindings insert a
// newline atomically (no stray backslash + submit).
...
// Legacy multiline sequence used before the CSI u migration. Old keybindings
// that still send this will be auto-replaced on the next terminal setup.
const LEGACY_MULTILINE_SEQUENCE = '\\\r\n'

/**
 * Migrate legacy keybindings that used the old \\\r\n escape sequence for
 * modified Enter keys. Those bytes arrived as separate key events (backslash +
 * return), causing unintended submissions; the replacement CSI u sequences
 * parse as one modified-return event.
 */
  ...
  try {
    ...
    try {
      ...
      if (code !== 'ENOENT') {
        return {
          success: false,
          message: `Failed to read ${meta.label} keybindings: ${String(error)}`
        }
      }
    }
    ...
    return {
      success: false,
      message: `Failed to configure ${meta.label} terminal shortcuts: ${String(error)}`
    }
  }
}
