...
class SessionUsageMixin:
    ...
    def record_model_activation(
        ...
        """Record one successful picker activation for this profile."""
        ...
        def _do(conn):
            ...
            row = conn.execute(
                """SELECT provider_id, model, base_url, last_used_at, activation_count
    ...
    @staticmethod
    def _model_picker_limit(limit: int) -> int:
        try:
            return int(limit)
        except (TypeError, ValueError):
            return 5

    def _list_picker_models(self, *, limit: int, order_by: str) -> List[Dict[str, Any]]:
        row_limit = self._model_picker_limit(limit)
        if row_limit <= 0:
            return []
        with self._lock:
            rows = self._conn.execute(
                f"""SELECT provider_id, model, base_url, last_used_at, activation_count
                    FROM model_picker_usage
                    ORDER BY {order_by}, provider_id ASC, model ASC, base_url ASC
                    LIMIT ?""",
                (row_limit,),
            ).fetchall()
    ...
    def list_recent_models(self, limit: int = 5) -> List[Dict[str, Any]]:
        """Return model identities ordered by their latest activation or API call."""
        return self._list_picker_models(
            limit=limit,
            order_by="last_used_at DESC, activation_count DESC",
        )

    def list_frequent_models(self, limit: int = 5) -> List[Dict[str, Any]]:
        """Return model identities ordered by real use plus successful activations."""
        return self._list_picker_models(
            limit=limit,
            order_by="activation_count DESC, last_used_at DESC",
        )
