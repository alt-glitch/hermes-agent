...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    *resolve_all* resolves every pending approval (``/approve all``); otherwise the oldest
    ...
    with _lock:
        ...
        if not queue:
            _gateway_queues.pop(session_key, None)
        # Popping the entry and committing its outcome are one critical section: a choice acknowledged to the client
        # cannot be popped-and-lost as a timeout. Stamp the fork lifecycle outcome in the same winner section.
...
def withdraw_gateway_approval(session_key: str, request_id: str, cause: str) -> bool:
    ...
    with _lock:
        ...
        entry.terminal_status = "cancelled"
        entry.event.set()
    _emit_gateway_lifecycle([entry], "cancelled")
    return True
...
def clear_session(session_key: str) -> None:
    ...
    with _lock:
        ...
        entries = _gateway_queues.pop(session_key, [])
        for entry in entries:
            # Cancel blocked waits now so the old run unwinds instead of idling until timeout. This is a withdrawn
            # prompt, not a user denial; keep the fork's correlated lifecycle terminal event.
