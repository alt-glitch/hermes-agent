...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _attribution and isinstance(_out, str) and len(_out) > 600:
        _out = (
            ...
            + _out[-600:])
    retrieval = ""
    if not _attribution and evt.get("output_cut"):
        # Keep upstream's explicit cut wording and the fork's richer retained-size/navigation hint.
        _out = (
            f"...(first {evt['output_cut']} characters cut — process(action=\"log\", "
        ...
        shown = len(str(evt.get("output") or ""))
        retrieval = (
            f"[Showing the final {shown:,} of {retained_label} retained characters. "
            f"Retrieve the retained output with process(action='log', session_id='{_sid}').]\n"
        )
    elif not _attribution and evt.get("output_truncated"):
        retained = evt.get("output_retained_chars")
        retained_label = f"{retained:,}" if isinstance(retained, int) else "more"
        retrieval = (
