"""Ordered deferred batches re-enter normal dispatch under each entry's real name."""
...
def _dispatch_connector_batch(calls, ids, *, user_task, enabled_tools,
    ...
    # Unlike upstream's connector-only batch guard, the fork intentionally preserves ordered local and mixed batches.
    # Local entries are validated against the live session and re-enter their normal policy pipeline below.
    ...
    for position in range(len(calls)):
        if position in claimed:
            continue
        if is_interrupted():
            # This bridge batch is one outer tool call, so check between every ordered entry and leave all unstarted
            # local and remote side effects explicitly interrupted.
            entries.extend(_interrupted_entries(calls, position, claimed))
            break
...
def _remote_entry(position: int, name: str, payload: Any) -> dict[str, Any]:
    ...
    if isinstance(value, Mapping) and "error" in value:
        error = _error_slot(value)
        if error.get("code") == "CONNECTION_REQUIRED":
            ...
            error = render_connection_required(
                connector=str(error.get("connector") or getattr(parsed, "connector", "")),
                message=str(error.get("message") or "Connection required."),
                connect_url=error.get("connect_url"),
                hint=error.get("hint"),
                card=session_platform() == "desktop",
            )
        return {"index": position, "name": name, "error": error}
