...
# Fork aggregate-verdict memo and upstream's once-per-process core-drop diagnostics are independent layers.
_verdict_snapshot_cache: Dict[tuple, tuple[float, tuple]] = {}
_verdict_snapshot_epoch = 0
_check_fn_ever_good: Set[tuple[Callable, Optional[str]]] = set()
_check_fn_core_drop_warned: Set[tuple[Callable, Optional[str]]] = set()
...
def invalidate_check_fn_cache() -> None:
    """Drop availability and aggregate-verdict caches after config/auth changes."""
...
class ToolRegistry:
    ...
    def check_fn_verdict_snapshot(self, scope: Optional[str] = None) -> tuple:
        """Hashable availability verdicts for outer memo keys.

        Registry generation covers mutations, not credentials or daemons changing state.
        A short aggregate TTL keeps the hot path cheap while ensuring outer caches re-key.
        """
        ...
        fns: Dict[Callable, object] = {}
        for entry in entries:
            if entry.check_fn is not None:
                fns.setdefault(entry.check_fn, _fn_label(entry.check_fn))
        for fn in toolset_checks.values():
            if fn is not None:
                fns.setdefault(fn, _fn_label(fn))
        verdicts = [(str(label), id(fn), _check_fn_cached(fn)) for fn, label in fns.items()
