...
class _ChildProgressRelay:
    ...
    def _on_complete(self, tool_name, preview, args, kwargs):
        # Preserve the fork's identity-aware lifecycle rendering (batch prefix and static task label), but route failed
        # diagnostics through upstream's notification boundary. The relayed event and child result are never gated.
        if kwargs.get("status") in SUBAGENT_FAILURE_STATUSES:
            from gateway.warning_notifications import render_notification
            parent = self.parent_scope
            render_notification(
                lambda: self._show_lifecycle(
                    "subagent.complete", preview, {**self._identity_kwargs(), **kwargs}
                ),
                platform=getattr(parent, "_notification_platform", getattr(parent, "platform", "cli")) if parent else "cli",
                user_config=getattr(parent, "_notification_config", None) if parent else None,
            )
