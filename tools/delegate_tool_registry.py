...
def list_active_subagents(parent_session_id: Optional[str] = None) -> List[Dict[str, Any]]:
    """Copy of the running tree, optionally scoped to one durable parent session."""
    with _active_subagents_lock:
        snapshots = []
        for record in _active_subagents.values():
            owner = record.get("owner_agent_session_id")
            if parent_session_id and owner != parent_session_id:
                continue
            snapshot = {k: v for k, v in record.items() if k not in _PRIVATE_RECORD_KEYS}
            # Stable public wire name retained for the TUI even though the
            # upstream registry owns it internally as owner_agent_session_id.
            snapshot["parent_session_id"] = owner
