...
class ProcessSession:
    ...
    # Finalization holds this fence across cleanup and durable persistence;
    # receipt serialization snapshots the same session while that fence is held.
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _move_to_finished(self, session: ProcessSession) -> bool:
        ...
        # ``_finished`` only after output cleanup and durable-result persistence.
        # Do not hold the registry lock while invoking the environment finalizer:
        # unrelated process lookups must not wait on CWD-marker parsing.
        with session._lock:
            with self._lock:
        ...
        if was_running and session.notify_on_complete:
            ...
            notification = {
                ...
                **output,
                # Fork UI/model consumers retain the richer truncation metadata while
                # upstream consumers use output_cut and per-process completion sizing.
                **({
                    "output_truncated": True,
                    "output_retained_chars": len(retained_output),
                } if "output_cut" in output else {}),
    ...
    def kill_process(
        ...
        try:
            ...
            if session.systemd_unit:
                _stop_systemd_unit(session.systemd_unit)
            # Post-kill verification: surviving descendants must remain visible and killable rather than being
            # hidden behind a false killed receipt.
            with session._lock:
            ...
            if survivors:
                ...
                return {
                    ...
                    "process_running": True}
            # Mark consumed before exposing ``exited`` to watcher tasks. Finalize before taking the returned snapshot:
            # yielded commands may still carry their environment's private CWD marker here.
            with session._lock:
                if consume_output:
                    self._completion_consumed.add(session_id)
                session.exited = True
            ...
            # The reader thread can finalise the session while the signal path blocks in the SIGKILL grace window:
            # rewrite the receipt when it won so the durable result matches what the caller was told.
    ...
    def list_sessions(self, task_id: str = None, session_key: str = None, *, include_retained: bool = False) -> list:
        ...
        for s in all_sessions:
            ...
            # Runs before taking s._lock: the drain path appends output under that lock.
            self._reconcile_local_exit(s)
            # Snapshot output and exit metadata under the same per-session fence used by finalization.
            with s._lock:
                entry = {
                    "session_id": s.id,
