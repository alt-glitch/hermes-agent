...
def should_activate(config: ToolSearchConfig, deferrable_tokens: int,
    ...
    if deferrable_tokens > 0:
        return True
    # A signed-in account with remote connectors still needs the bridge when
    # this session has no local MCP/plugin schemas to defer.
    try:
...
def _search_description(deferred_count: int, listing: Optional[str], listing_form: str) -> str:
    ...
    prefix = (
        ...
        "Search remote connector tools (email, calendars, issue trackers, and more) "
...
def bridge_tool_schemas(deferred_count: int, listing: Optional[str] = None,
    ...
    return [
        ...
        _bridge_schema(
            ...
            "entry per invocation; a single call is an array of one. Entries run as one "
            "batch and results return in the same order. Argument shapes match each "
            f"tool's schema (see `{TOOL_DESCRIBE_NAME}`). Policy, hooks, and approvals "
            "run exactly as for any directly-listed tool.",
            {
                "calls": {
                    "type": "array",
                    "items": {
                    ...
                    "description": "Tool invocations to run as one batch.",
                },
            },
            ["calls"],
        ),
...
def _connector_matches_by_group(queries: List[str], connector_search=None) -> Tuple[List[List[str]], Dict[str, Dict[str, Any]]]:
    """Remote connector matches aligned by query position; failure is a silent empty leg."""
...
def dispatch_tool_search(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    remote_matches, remote_records = _connector_matches_by_group(queries, connector_search)
...
def dispatch_tool_describe(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
                           config: Optional[ToolSearchConfig] = None, connector_describe=None) -> str:
    ...
    connector_names = [name for name in names if is_connector_name(name)]
    if connector_names:
        try:
            ...
            logger.debug("connector describe merge failed silently", exc_info=True)
...
def normalize_tool_call_entries(args: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """Normalize the advertised calls-array shape and the legacy single-call shape."""
    ...
    for position, raw in enumerate(raw_calls):
        ...
        if isinstance(raw_args, str):
            try:
                ...
                return [], f"tool_call calls[{position}].arguments is not valid JSON: {exc}"
...
def resolve_underlying_call(args: Dict[str, Any]) -> Tuple[Optional[str], Dict[str, Any], Optional[str]]:
    """Resolve one local call or a connector/multi-entry batch dispatch unit."""
...
__all__ = [
    ...
    "dispatch_tool_describe", "normalize_tool_call_entries", "resolve_underlying_call",
    "scoped_deferrable_names", "validate_deferred_call_args", "CONNECTOR_BATCH_SENTINEL",
    "is_connector_name"]
