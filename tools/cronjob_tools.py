...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "inactivity_timeout_seconds" in job:
        result["inactivity_timeout_seconds"] = job["inactivity_timeout_seconds"]
...
def _execute_job_now(
    ...
    return _run_claimed_job(
        claimed_job, extra_prompt=extra_prompt, execution_id=execution_id
    )
...
def _run_claimed_job(
    ...
    try:
        ...
        if not isinstance(run_claim, dict) or not run_claim.get("token"):
            refreshed_claim = get_job(job_id) or {}
            run_claim = refreshed_claim.get("run_claim")
        run_claim_token = (
            str(run_claim["token"])
        ...
        if not try_register_running_job(
            ...
            if execution_id is not None:
                ...
                _finish_execution_best_effort(
                    execution_id,
                    success=False,
                    error="In-flight registration rejected; execution was not started.",
                )
            return {
                "claimed": True,
                "success": False,
        ...
        try:
            try:
                dispatched_job = (
                    dict(job, execution_id=execution_id)
                    if execution_id is not None
                    else job
                )
                processed = run_one_job(
                    dispatched_job, adapters=adapters, loop=gateway_loop,
...
def _try_dispatch_background_run(
    ...
    try:
        ...
        # Persist the attempt before the CAS can advance a recurring schedule.
        execution_id = create_execution(job_id, source="manual")["id"]
        claimed_job = claim_job_for_fire(job_id, return_job=True)
        if not isinstance(claimed_job, dict):
            _finish_execution_best_effort(
    ...
    result = _run_claimed_job(
        job, extra_prompt=extra_prompt, execution_id=execution_id
    )
    result["dispatched"] = False
    return result
    ...
    try:
        ...
        if normalized == "update":
            ...
            if inactivity_timeout_seconds is not None:
                updates["inactivity_timeout_seconds"] = inactivity_timeout_seconds
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "inactivity_timeout_seconds": {
                ...
                "description": (
                    "Optional per-job agent inactivity watchdog in seconds. This "
                    "applies only to LLM-driven jobs (no_agent=False); script-only "
                    "jobs use cron.script_timeout_seconds instead. The timer resets "
                    "on model/tool/stream activity, so active jobs may run longer. "
                    "Use 0 for unlimited. On update, use 'inherit' to restore the "
                    "global default (600s unless configured)."
                ),
            },
            "workdir": {
                "type": "string",
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        inactivity_timeout_seconds=args.get("inactivity_timeout_seconds"),
