...
def test_session_branch_persists_branched_from_marker(server, monkeypatch):
    ...
    class _DB:
        ...
        def append_messages_batch(self, _session_id, _messages, **_kwargs):
            return []
    ...
    assert resp["result"]["session_key"] == new_key
...
def test_session_branch_with_count_truncates_history(server, monkeypatch):
    ...
    class _DB:
        ...
        def append_messages_batch(self, session_id, messages, **_kwargs):
            append_calls.extend(
                dict(message, session_id=session_id) for message in messages
            )
