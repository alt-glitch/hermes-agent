...
def test_concurrent_first_turn_keeps_one_lease_through_transfer_and_release(
    ...
    peer_entered_mutation = threading.Event()
    claimed = []
    ...
    class _ObservedMutationLock:
        ...
        def __enter__(self):
            ...
            self._entries += 1
            if self._entries == 2:
                peer_entered_mutation.set()
            return self

        def __exit__(self, *_args):
            self._lock.release()

    session["_mutation_lock"] = _ObservedMutationLock()

    def claim_together(*args, **kwargs):
        assert peer_entered_mutation.wait(timeout=5)
    ...
    try:
        ...
        assert claimed == [stored]
        assert not stored.released
...
def test_concurrent_first_turn_shares_refusal_and_retries_new_generation(monkeypatch):
    ...
    try:
        ...
        assert server._ensure_active_session_slot("live-sid", session) is None
        assert call_count == 2
...
def test_concurrent_first_turn_never_retains_unregistered_lease(
    monkeypatch, tmp_path
):
    """A stale registry observation cannot replace the durable winner."""
    ...
    real_claim = server._claim_active_session_slot
    real_snapshot = active_sessions.active_session_registry_snapshot
    ...
    def coordinated_claim(*args, **kwargs):
        ...
        with claim_lock:
            claim_index = claim_count
            claim_count += 1
        if claim_index > 0:
            assert first_snapshotted.wait(timeout=5)
        result = real_claim(*args, **kwargs)
        if claim_index == 0:
            first_claimed.set()
        return result

    def stale_first_snapshot(*args, **kwargs):
        ...
        if snapshot_index == 0:
            ...
            assert peer_finished.wait(timeout=5)
        return captured

    def peer_admission():
        try:
            return server._ensure_active_session_slot("live-sid", session)
        finally:
            peer_finished.set()

    monkeypatch.setattr(server, "_claim_active_session_slot", coordinated_claim)
    monkeypatch.setattr(
        active_sessions, "active_session_registry_snapshot", stale_first_snapshot
    )

    try:
        with ThreadPoolExecutor(max_workers=2) as pool:
            first = pool.submit(
                server._ensure_active_session_slot, "live-sid", session
            )
            assert first_claimed.wait(timeout=5)
            peer = pool.submit(peer_admission)
            assert peer.result(timeout=5) is None
            assert first.result(timeout=5) is None

        stored = session["active_session_lease"]
        registered_ids = {
            entry["lease_id"] for entry in real_snapshot()
        }
        assert stored.lease_id in registered_ids
        assert not stored.released
...
def test_compute_host_turn_end_updates_metadata_mirror(monkeypatch):
    ...
    try:
        ...
        # update_behind is populated by an independent background git probe;
        # it may legitimately move None -> 0 between these two consecutive
        # _session_info calls and is not part of the compute-host mirror.
        assert {
            key: (emitted[-1][2].get(key), info.get(key))
            for key in (emitted[-1][2].keys() | info.keys()) - {"update_behind"}
            if emitted[-1][2].get(key) != info.get(key)
        } == {}
    finally:
        server._sessions.pop("iso-sid", None)
...
def test_session_resume_uses_parent_lineage_for_display(monkeypatch, omit_messages):
    ...
    # Preserve the native engine's explicit verbose-tool projection while also
    # exercising the desktop's duplicate-transcript suppression contract.
    params = {"session_id": target, "with_tool_output": True}
    if omit_messages:
...
def test_cold_then_live_resume_keeps_verbatim_verification_history(
    ...
    """Cold and warm resume expose the same durable verification lineage."""
...
def test_config_set_reasoning_updates_live_session_and_agent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        server,
        "_emit",
        lambda event, sid, payload: events.append((event, sid, payload)),
    )
...
def test_prompt_submit_storage_rejection_rolls_back_admission_for_retry(
    monkeypatch,
):
    """A 5072 rejection must not leave the session permanently busy."""

    persist_results = iter([False, True])

    def _persist(session):
        result = next(persist_results)
        if result is False:
            session["queued_prompt"] = {
                "text": "queued while persistence failed",
                "client_submission_ids": ["queued-submit"],
            }
            session["queued_prompts"] = [{"text": "another queued prompt"}]
        return result
    ...
    session = _session(
        _active_client_submission_ids=["stale-active"],
        _pending_steer_submission_ids=["stale-steer"],
        pending_steer_chars=11,
    )
    server._sessions["storage-retry-sid"] = session
    try:
        rejected = server.handle_request(
            {
                ...
                "method": "prompt.submit",
                "params": {
                    "client_submission_id": "lost-submit",
                    "session_id": "storage-retry-sid",
                    "text": "will vanish",
                },
            }
        )

        assert rejected["error"]["code"] == 5072
        assert session["running"] is False
        ...
        assert session["pending_steer_chars"] == 0
        ...
        retry = server.handle_request(
            {
                ...
                "method": "prompt.submit",
                "params": {
                    "client_submission_id": "retry-submit",
                    "session_id": "storage-retry-sid",
                    "text": "try again",
                },
            }
        ...
        server._sessions.pop("storage-retry-sid", None)
...
def test_prompt_submit_clears_running_before_deferred_init_error(monkeypatch):
    ...
    The server must clear running before publishing the terminal error frame so
    a TUI can safely drain its bounded local queue from that event. A trailing
    stale ``session.info(false)`` would race the next optimistic submission, so
    this path intentionally emits no such snapshot.
    """
    ...
    terminal = next(
        (running, payload)
        for event, running, payload in emitted
        if event == "message.complete"
    )
    assert terminal[0] is False
    assert terminal[1]["client_submission_ids"] == [
        "direct-send",
        "steer-send",
    ]
    assert terminal[1]["status"] == "error"
    assert terminal[1]["error"] == "agent init failed"
    assert terminal[1]["recoverable"] is True
...
def test_session_list_no_params_keeps_legacy_shape_with_widened_rows(monkeypatch):
    db = _picker_db()
    monkeypatch.setattr(server, "_get_db", lambda: db)
    ...
    # Legacy response shape: last-active DESC, `tool` denied, one compact DB fetch.
...
def test_session_list_single_source_passes_through_to_db(monkeypatch):
    ...
    assert db.calls[0]["compact_rows"] is True
...
def test_make_agent_applies_inherited_fast_request_overrides(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.models.resolve_fast_mode_overrides",
        lambda model, **_kwargs: {"service_tier": "priority", "speed": "fast"}
        if model == "test-model"
        else None,
    )
