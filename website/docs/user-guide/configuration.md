...
If a terminal UI loses the acknowledgement for a prompt response, it keeps the prompt visible as **delivery not confirmed** and never resends automatically. Esc or Ctrl+C can dismiss that local card, but the UI will not claim that a cancellation or denial reached the gateway.
