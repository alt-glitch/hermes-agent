...
async function ensureRegistryBackend(
  ...
  opts: { localRoute?: RegistryLocalRoute; passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}
) {
  const spawnPriority = spawnPriorityFrom(opts.spawnPriority)
  const passive = Boolean(opts.passive)
  const registry = readDesktopConnectionsRegistry()
...
async function connectDesktopProfileRoute(
  ...
  // both land here. Registry-local forced children use the exact pool key that
  // receives the spawn, so foreground intent cannot be stranded on the
  // connection/profile alias while the real child waits as background work.
  let localRoute: RegistryLocalRoute | null = null

  if (route.connectionId) {
    const registry = readDesktopConnectionsRegistry()
    const source = registry.connections.find(connection => connection.id === route.connectionId)
    localRoute = source?.kind === 'local' ? resolveCurrentRegistryLocalRoute(route.profile) : null
  }

  const scopeKey = registryBackendDialScopeKey(route.connectionId, route.profile, localRoute)
  ...
  try {
    connection = await backendDialClaims.run(scopeKey, () =>
      route.connectionId
        ? ensureRegistryBackend(route.connectionId, route.profile, '', {
