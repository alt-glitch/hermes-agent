...
class CLILoopsMixin:
    ...
    def _maybe_fire_loop_tick(self) -> None:
        ...
        if wakeup.lstrip().startswith("/"):
            try:
                _print_decision_message(mgr.complete_tick("", claim_id))
                self._active_loop_claim_id = ""
            except Exception:
                pass
    ...
    def _maybe_complete_loop_tick_after_turn(self) -> None:
        ...
        if getattr(self, "_last_loop_turn_failed", False):
            ...
            _cprint(f"  {_DIM}↻ Loop wakeup failed — it will retry on the next cadence.{_RST}")
            return
        ...
        if getattr(self, "_last_turn_interrupted", False):
            ...
            return
        decision = mgr.complete_tick(self._last_assistant_response_text(), claim_id)
        self._active_loop_claim_id = ""
