...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        fetch_error = (fetch_result.stderr or "").lower()
        missing_remote_ref = (
            "couldn't find remote ref" in fetch_error
            or "could not find remote ref" in fetch_error
        )
        if fetch_result.returncode != 0 and not branch_explicit and branch != "main" and missing_remote_ref:
        ...
        verify_result = _git_run(git_cmd, ["rev-parse", "--verify", "--quiet", f"origin/{branch}"])
        if verify_result.returncode != 0 and not branch_explicit and branch != "main":
            print(f"  ⚠ Branch '{branch}' is local-only — falling back to main...")
            branch = "main"
            fetch_result = _git_run(git_cmd, ["fetch", "origin", branch], network=True)
            if fetch_result.returncode != 0:
                _print_fetch_failure(fetch_result.stderr)
                sys.exit(1)
            verify_result = _git_run(git_cmd, ["rev-parse", "--verify", "--quiet", "origin/main"])
        if verify_result.returncode != 0:
