...
def _commit_model_switch(
    ...
    snapshot = None
    if one_turn:
        # Repeated --once switches before the next prompt still restore the
        # runtime that preceded the first temporary switch.
        snapshot = getattr(cli, "_pending_one_turn_model_restore", None)
        if snapshot is None:
