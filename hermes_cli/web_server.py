...
def _resolve_chat_argv(
    ...
    # Capture the dashboard launch profile's config before switching the
    # context-local Hermes home. Only env vars owned by that config are removed
    # below; operator-exported fallbacks remain available to the selected profile.
    ...
    if profile_dir is not None:
        try:
            ...
            _log.warning("Failed to read dashboard terminal config", exc_info=True)
    ...
    try:
        ...
        try:
            ...
            if profile_dir is not None:
                # build_subprocess_env inherited the dashboard launch profile's
                # bridged terminal values. Remove only values that launch config
                # owned, then apply the selected profile under its active home.
