...
DEFAULT_CONFIG = {
    ...
    "tools": {
        ...
        "connectors": {
            # Signed-out users and gateways without connector routes degrade
            # to the local tool catalog.
            "enabled": True,
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_URL": _tool(
        "Exact shared tool-gateway origin override for Nous Subscribers only (serves on-origin "
        "managed vendors and media uploads), e.g. http://127.0.0.1:3009 for local testing",
        "Shared tool-gateway URL (leave empty to derive from domain)", None, password=False,
        advanced=True),
    "CONNECTOR_GATEWAY_URL": _tool(
        "Exact connector-gateway origin override for Nous Subscribers only (serves the "
        "connectors API), e.g. http://127.0.0.1:3009 for local testing",
        "Connector-gateway URL (leave empty to derive from domain)", None, password=False,
        advanced=True),
    "TOOL_GATEWAY_DOMAIN": _tool(
        "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor "
        "passthroughs, the shared managed origin, and the connectors API",
