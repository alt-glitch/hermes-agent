...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_URL": {
        "description": "Exact shared tool-gateway origin override for Nous Subscribers only (serves on-origin managed vendors and media uploads), e.g. http://127.0.0.1:3009 for a local gateway (optional)",
    ...
    "CONNECTOR_GATEWAY_URL": {
        "description": "Exact connector-gateway origin override for Nous Subscribers only (serves the connectors API), e.g. http://127.0.0.1:3009 for a local gateway (optional)",
        "prompt": "Connector-gateway URL (leave empty to derive from domain)",
        "url": None,
        "password": False,
        "category": "tool",
        "advanced": True,
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive every host shape: per-vendor passthroughs ({vendor}-gateway.<domain>, e.g. firecrawl-gateway.nousresearch.com), the shared managed origin (tool-gateway.<domain>), and the connectors API (connector-gateway.<domain>)",
