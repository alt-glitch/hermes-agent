...
def _shutdown_recorder(rec: Any, *, context: str) -> None:
    """Best-effort release of a recorder's persistent microphone stream.

    ``stop()`` and ``cancel()`` keep the stream reusable for an active continuous loop. Once
    ownership ends, callers must explicitly shut it down or macOS keeps reporting microphone
    use while the TUI is otherwise idle.
    """
    if rec is None:
        return
    try:
...
def _release_continuous_recorder(rec: Any, *, context: str) -> None:
    """Detach ``rec`` from process state and release it exactly once."""
    global _continuous_recorder
    with _continuous_lock:
        if _continuous_recorder is not rec:
            return
        _continuous_recorder = None
    _shutdown_recorder(rec, context=context)
...
def start_continuous(
    ...
    try:
        ...
        _deactivate()
        _release_continuous_recorder(rec, context="continuous start failure")
        raise
...
def stop_continuous(force_transcribe: bool = False) -> None:
    ...
    with _continuous_lock:
        ...
        track_no_speech = was_active and force_transcribe and not _continuous_auto_restart
        _continuous_stopping = was_active and force_transcribe and rec is not None
    ...
    if rec is not None:
        if was_active and force_transcribe and on_transcript:
            ...
            threading.Thread(
                target=lambda: _finish_forced_stop(wav_path, callbacks, track_no_speech), daemon=True
            ).start()
            return
...
def _continuous_on_silence() -> None:
    ...
    wav_path = rec.stop()
    if not auto_restart:
        # The TUI's client-driven mode starts a fresh recorder for each utterance. Release this
        # stream before network transcription rather than leaving CoreAudio open while idle.
        _release_continuous_recorder(rec, context="one-shot silence stop")
    ...
    if stop_phrase or limit_hit:
        ...
        _release_continuous_recorder(rec, context="continuous loop halt")
        _safe_call(on_status, "idle")
        return
...
def _rearm_after_turn(rec: Any, on_status, no_speech: int) -> None:
    ...
    try:
        ...
        _deactivate(on_status)
        _release_continuous_recorder(rec, context="continuous restart failure")
        return
    _safe_call(on_status, "listening")
