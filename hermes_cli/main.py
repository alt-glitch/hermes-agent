...
def __getattr__(name):
    ...
    if module is not None:
        ...
        if module == "hermes_cli.update_cmd":
            imported = _install_opentui_update_wrapper(imported)
...
# Upstream mechanically extracted the update pipeline while the fork added
# transactional OpenTUI hydration in the former in-file implementation. Keep
# the extracted implementation authoritative, then decorate its Node refresh
# at the module seam so both the real update path and legacy main.py test hooks
# retain the fork behavior.
def _install_opentui_update_wrapper(update_cmd):
    """Lazily decorate upstream's dependency refresh with OpenTUI hydration."""
    current = update_cmd._update_node_dependencies
    if getattr(current, "_hermes_opentui_refresh", False):
        return update_cmd

    workspace_refresh = getattr(current, "__wrapped__", current)

    @_functools.wraps(workspace_refresh)
    def _update_node_dependencies() -> list[str]:
        failures = workspace_refresh() or []
        if _update_opentui_package() is False:
            failures.append("OpenTUI engine")
        return failures

    _update_node_dependencies._hermes_opentui_refresh = True
...
def _finalize_update_output(state):
    """Restore stdio and close the update log opened for hangup protection."""
    if not state:
        return
...
def cmd_update(args):
    ...
    update_io_state = _install_hangup_protection(gateway_mode=gateway_mode)
    ...
    if not update_lock.acquire():
        ...
        _finalize_update_output(update_io_state)
...
def cmd_dashboard(args):
    ...
    # Dashboard/serve agents and cron ticks run in-process, so without this
    # bridge a configured non-local backend can silently fall back to host
    # execution (#63141, #54449, #61115, #65696).
    try:
        from hermes_cli.config import apply_terminal_config_to_env
        ...
        logger.debug(
            "terminal config → env bridge failed for dashboard/serve",
            exc_info=True,
        )
