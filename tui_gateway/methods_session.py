...
def _correction_method(name: str, verb: str, accepted_status: str, supported, unsupported: str):
    ...
    def _(rid, params: dict) -> dict:
        ...
        if verb == "steer":
            ...
            return _ok(
                rid,
                {"status": accepted_status if accepted else "rejected", "text": text},
            )
...
def _(rid, params: dict) -> dict:
    ...
    parent_session_id = str(
        getattr((session or {}).get("agent"), "session_id", "") or "") or None
    active = (
        dt.list_active_subagents(parent_session_id=parent_session_id)
        if parent_session_id is not None else [])
    return _ok(rid, {"active": active,
                     "paused": dt.is_spawn_paused(),
