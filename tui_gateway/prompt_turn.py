    ...
    with _session_mutation_lock(session):
        ...
        with session["history_lock"]:
            ...
            if not isinstance(inflight, dict) or inflight.get("status") == "error":
                _start_inflight_turn(session, text, display_kind=display_kind, display_metadata=display_metadata)
            with contextlib.suppress(Exception):
                _clear_agent_interrupt_for_turn(session, agent)
            active_ids = list(session.get("_active_client_submission_ids") or [])
            session["_active_client_submission_ids"] = list(dict.fromkeys([*active_ids, *client_submission_ids]))
...
def _settle_loop_claim(sid: str, session: dict, raw: Any, status: str, claim_id: str) -> None:
    """Complete only the tick this turn owns; release it on every non-success outcome."""
    if not claim_id:
        return
    try:
        ...
        if not loop_sid_key:
            return
        mgr = LoopManager(session_id=loop_sid_key)
        if status == "complete":
            decision = mgr.complete_tick(raw if isinstance(raw, str) else "", claim_id)
            if message := decision.get("message") or "":
                _emit("status.update", sid, {"kind": "loop", "text": message})
        else:
            mgr.abandon_tick(claim_id)
    except Exception as exc:
        _hook_failure("loop completion hook", exc)
...
def _complete_turn_payload(
    ...
    final_text = result_fields.get("final_response")
    has_partial_text = bool(
        result_fields.get("partial") and isinstance(final_text, str)
...
def _seal_turn_steers(session: dict, agent: Any, result: Any) -> tuple[bool, list[str]]:
    """Close steer admission and move any too-late steer to the next turn."""
    leftover = (
        result.get("pending_steer")
        if isinstance(result, dict) and isinstance(result.get("pending_steer"), str)
        else None
    )
    with session["history_lock"]:
        ...
        try:
            ...
            post_finalizer = drain_steer() if callable(drain_steer) else None
        except Exception:
            post_finalizer = None
        if post_finalizer:
            leftover = f"{leftover}\n{post_finalizer}" if leftover else post_finalizer
        steer_ids = list(session.get("_pending_steer_submission_ids") or [])
        retained = bool(
            leftover
            and not _promote_leftover_steer(session, agent, leftover, steer_ids)
        )
        if not retained:
...
def _recover_turn_exception(sid: str, session: dict, st: _TurnRun, e: BaseException) -> None:
    ...
    leftover_steer_retained, failed_submission_ids = _seal_turn_steers(
        session, st.agent, st.result)
    if leftover_steer_retained:
        _emit("error", sid, {"message": (
...
def _run_prompt_submit(
    ...
    # Every synthesized turn must materialize the owning session row too.
    ...
    admitted = _admit_prompt_turn(
        sid, session, text, image_paths, queued_prompt_generation, client_submission_ids,
        display_kind, display_metadata, display_notification)
    if admitted is None:
        return False
    ...
    def run_body():
        ...
        try:
            ...
            if loop_claim_id and not loop_claim_settled:
                _settle_loop_claim(sid, session, "", "error", loop_claim_id)
        ...
        if not history_commit_reported:
            _report_history_commit(history_commit_callback, _HistoryCommitOutcome(
                display_persisted=False, history_retained=False,
                invocation_started=st.invocation_started))
        return st.result, goal_followup

    def run():
        from agent.notification_presentation import notification_policy_snapshot, notification_turn
        with notification_policy_snapshot(agent, "tui", notification_config), notification_turn(
                agent, muted=muted, session_id=sid):
            followup = run_body()
