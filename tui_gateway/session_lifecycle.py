...
def _session_is_detached(sid: str, session: dict) -> bool:
    """Detect lifecycle detachment while tolerating direct helper callers.

    Production sessions are registry-owned. Focused tests may call turn helpers
    with a never-registered record, so an absent entry alone is not proof of
    teardown. A finalized/stamped record or a replacement entry is stable proof.
    """
...
def _session_mutation_lock(session: dict):
    """Return the lazily-installed lock for destructive session mutations."""
...
def _pop_session_by_id(
    ...
    lock while waiting so one slow session cannot stall unrelated lifecycle work.
    """
    with _sessions_lock:
...
def _close_sessions_for_transport(transport, *, end_reason: str = "ws_disconnect") -> tuple[int, int]:
    ...
    for sid, session in clientless:
        ...
        # waiting for a turn's mutation lock. Resume attachment uses this same outer lock.
