...
def _persist_session_row_for_submit(rid, sid, session, text=None, display_kind=None):
    ...
    # and terminally settle any concurrently accepted inputs.
...
def _lock_in_submit_turn(
    ...
    with session["history_lock"]:
        if session.get("_tools_configuring"):
            return _err(
                rid, 4009,
                "session tools are being reconfigured — wait for it to finish"), fields
...
def _(rid, params: dict) -> dict:
    ...
    # rebuild and then start a turn that should have been rejected as racing it.
    with session["history_lock"]:
        if session.get("_tools_configuring"):
            return _err(
                rid, 4009,
    ...
    with _try_mcp_turn_admission(session, sid=sid) as admitted:
        if not admitted:
            ...
            return _err(
                rid, 4009, "MCP reload in progress — retry the prompt when it finishes",
                {"kind": "mcp_reload_in_progress", "retry_after_ms": 250})
        while True:
            with session["history_lock"]:
                ...
                if has_truncation:
                    # Rewinds must retain their history cut instead of becoming a
                    # steer/plain queued follow-up; the client retries after interrupt.
        ...
        requested_rebind_ids = (
            {value for value in raw_rebind_ids
    ...
    if (err := _persist_session_row_for_submit(rid, sid, session, text, display_kind)) is not None:
        return err
