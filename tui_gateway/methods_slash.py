...
def _mirror_reload_mcp(sid, session, agent, arg) -> str:
    """Reload the gateway-process MCP registry and the live agent snapshot."""
    if not agent:
        return ""
    try:
        with _mcp_registry_transition_lock, _mcp_reload_admission_lock:
            with _sessions_lock:
                busy_sid = next(
                    (live_sid for live_sid, live in _sessions.items() if live.get("running")),
                    None)
                detached_request_busy = (
                    session.get("running") and _sessions.get(sid) is not session)
            if busy_sid is not None or detached_request_busy:
                return "session busy — wait for every live turn to finish before reloading MCP"
            mcp_agent, mcp_lifecycle, mcp_discovery = (
                _tools_mod("tools.mcp_tool_agent"),
                _tools_mod("tools.mcp_tool_lifecycle"),
                _tools_mod("tools.mcp_tool_discovery"))
            mcp_lifecycle.shutdown_mcp_servers()
        ...
        return f"live session sync failed: {exc}"
    return ""
