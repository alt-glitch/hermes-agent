...
def _set_fast(rid, params, key, value, session):
    ...
    if session is not None:
        ...
        pinned_tier = {"fast": "priority", "normal": ""}.get(nv, nv)
        ...
        with runtime_lock:
            ...
            # Deferred construction may have published after the snapshot above.
            agent = session.get("agent")
            if agent is not None:
    ...
    if agent is not None:
        if session is None:
            _apply_service_tier_override(agent, {"fast": "priority", "normal": ""}.get(nv, nv))
        _persist_live_session_runtime(session)
...
def _set_reasoning(rid, params, key, value, session):
    ...
    if session is not None:
        # Serialize with deferred agent publication. Otherwise a mid-build
        # /reasoning can update the pin while the builder publishes an agent
        # carrying the previous effort, leaving status and requests divergent.
        runtime_lock = session.setdefault("runtime_override_lock", threading.Lock())
        with runtime_lock:
            resume_overrides = session.get("resume_runtime_overrides")
            if scope == "global":
                # A global choice is the new baseline, not a session pin.
                session.pop("create_reasoning_override", None)
                if isinstance(resume_overrides, dict):
                    resume_overrides.pop("reasoning_config_override", None)
            else:
                # Session-scoped like the gateway's `/reasoning <level>`; a
                # menu pick must not rewrite global config.
        ...
        # A deferred/lazy agent may take seconds to publish. Repaint the
        # session-scoped status field immediately; the eventual full
        # session.info will reaffirm the same value.
        _emit(
            "session.info",
