...
def _notif_dispatch_completions(sid, session, notifications, registry, deferred):
    ...
    if len(claimed) == 1:
        kwargs = _notification_turn_display(claimed[0][0], text, sid)
    else:
        failed = any(int(event.get("exit_code") or 0) != 0 for event, _text, _claim in claimed)
        notice = {
            "always_visible": True,
            "key": "proc-batch:" + ":".join(str(event.get("session_id") or "") for event, _, _ in claimed),
            "kind": "process.complete",
            "level": "warn" if failed else "info",
            "text": f"{len(claimed)} background processes completed",
            "detail": text,
        }
        kwargs = {
            "display_kind": "process_complete",
            "display_metadata": {key: value for key, value in notice.items() if key != "detail"},
    ...
    if card_pending:
        for event, _text, _claim in claimed:
            event["_tui_notification_shown_for"] = sid

    def settle_batch(outcome):
        for event, _text, claim in claimed:
            _settle_notification_delivery(sid, event, claim, outcome)

    kwargs["history_commit_callback"] = settle_batch

    def retry_batch():
        for event, _text, claim in claimed:
            if card_pending:
                event.pop("_tui_notification_shown_for", None)
    ...
    try:
        admitted = _notif_submit(
            ...
            "completion batch dispatch failed", **kwargs)
    except Exception:
        retry_batch()
        return
    if admitted is False:
