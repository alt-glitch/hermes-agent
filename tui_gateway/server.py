...
def _(rid, params: dict) -> dict:
    ...
    # A typed bare stop phrase is the keyboard twin of the spoken stop phrase
    # while backend voice mode is active. Consume it before session lookup so
    # it cannot start an agent turn.
            ...
            if (db := _get_db()) is not None:
                try:
                    ...
                    logger.error(
                        ...
                        "(ordinal=%d); refusing turn so memory and DB stay aligned: %s",
                        sid,
                        ordinal,
                        exc,
                        exc_info=True,
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    def _title_callback(title: str, key: str = _title_key) -> None:
                        # The native TUI derives its OSC title from session.info;
                        # the desktop/sidebar consumes the dedicated title event.
                        # Auto-title runs on a worker thread, and _emit serializes
                        # writes on the transport lock.
                        _emit_title_refresh(sid)
                        _emit(
                            "session.title",
                            sid,
                            {"session_id": key, "title": title},
                        )
