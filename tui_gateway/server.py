...
_LONG_HANDLERS = frozenset(
    {
        ...
        "model.options",
        # NOTE(defer-merge): upstream additions below are kept verbatim, but
        # pet.* / llm.oneshot RPCs are NOT yet implemented in the OpenTUI engine
        # — glitch must port them before they do anything here.
        "llm.oneshot",
        "pet.cells",
        "pet.gallery",
        "pet.generate",
...
def _wire_callbacks(sid: str):
    ...
    # NOTE(defer-merge): glitch's branch had REMOVED the timeout=120 below;
    # upstream kept it AND added the project-workspace callback. Upstream's
    # superset is taken here provisionally — glitch must confirm whether the
    # sudo timeout removal was intentional and should win instead.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # NOTE(defer-merge): glitch's filtered/paginated rewrite (below) is kept.
        # Upstream changed only the LEGACY fetch to pass order_by_last_active=True;
        # that tweak is NOT folded into glitch's legacy branch here — glitch must
        # decide whether the legacy unfiltered path should order_by_last_active.
        try:
            ...
            if (
                ...
                try:
                    ...
                    maybe_auto_title(
                        ...
                        raw,
                        session.get("history", []),
                        # NOTE(defer-merge): glitch's title_callback (OSC
                        # window-title refresh) is kept; upstream added a
                        # different session.title sidebar-rename callback that
                        # is NOT taken here — glitch must reconcile the two.
