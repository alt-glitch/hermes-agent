...
def _pop_session_by_id(sid: str) -> dict | None:
    ...
    Destructive agent replacement is serialized by the per-session mutation
    lock. The registry lock is released while waiting for that lock so one
    slow session cannot stall unrelated session lifecycle operations.
    """
    with _sessions_lock:
...
def _terminal_task_cwd(session: dict | None) -> str:
    ...
    When ``TERMINAL_ENV`` is unset, consult ``terminal.backend`` as a fallback.
...
def _persist_branch_seed(session: dict) -> None:
    ...
    with _session_db(session) as db:
        ...
        try:
            ...
            append_batch = getattr(db, "append_messages_batch", None)
            if callable(append_batch):
                append_batch(key, messages, chunk_rows=500)
            else:
                for message in messages:
                    db.append_message(session_id=key, **message)
...
def _is_display_hidden_marker(role: str | None, text: str) -> bool:
    """Hide model-facing ``[System: …]`` bookkeeping from every transcript."""
...
def _history_to_messages(
    ...
    # ``include_tool_output`` is an OpenTUI opt-in: the native engine renders
    # capped tool history collapsed, while Ink keeps its memory-safe default.
    messages = []
    tool_call_args = {}
...
def _interrupt_busy_session(
    ...
    *,
    ...
    network call returns. Run that call on one bounded worker per session. The
    prompt admission path already owns ``history_lock``; direct helper callers
    do not, so the pending flag supports both lock contracts.
    """
...
def _handle_busy_submit(
    ...
    *,
    ...
    fork defaults to a lossless next-turn queue. Explicit ``interrupt`` mode
    redirects capable core agents in place, falling back to hard interrupt plus
    queue; ``steer`` injects after the current atomic action and otherwise
    queues.
    ...
    with lock_context:
        ...
        # prompt.submit may already own history_lock through the MCP admission
            ...
            try:
                correlated_ids = list(
                    dict.fromkeys(
                        [
                            *list(
                                session.get("_active_client_submission_ids") or []
                            ),
                            *list(
        ...
        try:
            if image_paths:
                # Claim attachments at admission time. A later paste must not
                # be consumed by this queued prompt after the active turn ends.
                session["attached_images"] = []
            _enqueue_prompt(
                session,
                text,
    ...
    if should_interrupt:
        _interrupt_busy_session(
            ...
            session,
            agent,
            history_lock_owned=history_lock_owned,
        )
    return _ok(rid, {"status": "queued"})


def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    """Fire the accepted head prompt when idle; return whether it was claimed."""
    ...
    try:
        if use_compute_host:
            ...
            if client_submission_ids:
                submit_kwargs["client_submission_ids"] = client_submission_ids
            _run_prompt_submit(rid, sid, session, queued["text"], **submit_kwargs)
    except Exception as exc:
        print(
...
def _emit_terminal_turn_error(
    ...
    *,
    ...
    # desktop can name the failing layer and offer matching recovery actions.
    # This is advisory and must never replace the original terminal failure.
    ...
    if client_submission_ids:
        payload["client_submission_ids"] = list(
            dict.fromkeys(client_submission_ids)
        )
    if error_surface:
...
def _(rid, params: dict) -> dict:
    ...
    Optional params — omitting all of them keeps the legacy response shape
    (most-recently-active first, ``tool`` rows denied, default limit 200):
...
def _(rid, params: dict) -> dict:
    ...
    if is_truthy_value(params.get("lazy", False)):
        sid = uuid.uuid4().hex[:8]
        source = _resolve_session_source(str(params.get("source") or "").strip() or None)
        lease = None  # claimed lazily on the first turn
        try:
        ...
        # rows persisted by #65919 survive in the watch window. The repaired
        # ``history`` above still feeds live replay. Fall back to it if the
        # display read fails.
        try:
...
def _live_session_payload(
    ...
    *,
    ...
    with session["history_lock"]:
        ...
        reconcile_from_db = durable_display is None
        inflight = _inflight_snapshot(session)
        queued = _queued_prompt_snapshot(session)
    ...
    if reconcile_from_db and not omit_messages:
        ...
        # launch profile's state.db. A durable-but-changed prefix deliberately
...
def _(rid, params: dict) -> dict:
    ...
    with _session_db(session) as db:
        ...
        def _visible_branch_history(messages):
            ...
            for message in messages or []:
                if (
                    ...
                    or not _coerce_message_text(message.get("content")).strip()
                ):
                    continue
                visible.append(dict(message))
            return visible

        # The model projection may have been compacted to a summary and tail.
        # Branch from the persisted display projection so the child retains
        # the complete visible transcript, then reconcile any unflushed tail.
        history = None
        ...
        if callable(get_resume_conversations):
            try:
                ...
                history = _visible_branch_history(
                    _reconcile_display_with_live(
                        display_history,
                        in_memory_history,
                    )
                )
        ...
        source = _session_source(session)
        lease = None  # claimed lazily on the branch's first real turn
    ...
    try:
        ...
        try:
            ...
            # The live agent now owns the dedicated profile handle and will
            # close it during session teardown. Until this point every failure
            # remains this handler's responsibility.
    ...
    return _ok(
        ...
        {
            "session_id": new_sid,
            # Keep the native OpenTUI resume identity while exposing the shared
            # stored-session field consumed by Desktop and newer clients.
            "session_key": new_key,
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        # Leftover steer was already atomically promoted while admission was
        # fenced above. Re-enqueuing result["pending_steer"] here duplicates
        # the text and can overflow a queue whose preservation fallback already
        # restored the steer to the agent.
            ...
            if session is not None:
                ...
                with runtime_lock:
                    ...
                    if global_scope:
                        session.pop("create_reasoning_override", None)
                        if isinstance(resume_overrides, dict):
                            resume_overrides.pop("reasoning_config_override", None)
                    else:
                        # Session-scoped, like the messaging gateway's
                        # `/reasoning <level>`. Global persistence remains an
                        # explicit `--global` / Settings action.
...
def _(rid, params: dict) -> dict:
    ...
    if key == "indicator":
        ...
        return _ok(
            ...
            {
                "value": (
                    norm if norm in INDICATOR_STYLES else DEFAULT_INDICATOR_STYLE
                )
            },
        )
    if key == "personality":
...
def _(rid, params: dict) -> dict:
    ...
    class _ReloadBusy(RuntimeError):
        pass

    def _busy_session_id() -> str | None:
        with _sessions_lock:
            return next(
                (sid for sid, live in _sessions.items() if live.get("running")),
                None,
            )
    ...
    if _busy_session_id() is not None:
        ...
        # reading its cached tool snapshot. This early check gives a fast error;
        # the fenced checks below close the prompt-admission race.
        return _busy_error()
    try:
        ...
        def _do_full_reload() -> None:
            ...
            # Preserve the fork's admission contract inside upstream's
            # coalescing lock. Taking these fences here (rather than decorating
            # the whole handler) lets concurrent reload followers observe the
            # completed generation while still excluding prompt admission,
            # agent construction, and tool reconfiguration from global MCP
            # mutation.
            with _mcp_registry_transition_lock, _mcp_reload_admission_lock:
                if _busy_session_id() is not None:
                    raise _ReloadBusy
...
def _skill_only_slash_items(text: str) -> list[dict]:
    """Enumerate inline-reference completions from authoritative sources.

    Core commands and aliases retain dispatch precedence over bundles and
    skills; bundles retain their existing precedence over same-named skills.
    Bypassing the mixed completer prevents its first-30 built-in cap from
    hiding every skill from a bare inline ``/`` query.
    """
    ...
    def add_source(source, *, bundle: bool) -> None:
        for raw_key, raw_info in source.items():
            command = str(raw_key).strip()
            name = command.lstrip("/")
            folded = name.lower()
            if (
                not name
                ...
                or not folded.startswith(prefix)
                or folded in seen
                or resolve_command(name) is not None
            ):
                continue

            info = raw_info if isinstance(raw_info, dict) else {}
            default_description = "Skill bundle" if bundle else "Skill command"
            description = str(info.get("description") or default_description)
            short_desc = description[:50] + ("..." if len(description) > 50 else "")
            if bundle:
                meta = f"▣ {short_desc} ({len(info.get('skills', []))} skills)"
            else:
                meta = f"⚡ {short_desc}"
            items.append(
                {
    ...
    add_source(get_skill_bundles(), bundle=True)
    add_source(get_skill_commands(), bundle=False)
    return items[:30]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if params.get("skills_only") is True:
            return _ok(
                rid,
                {"items": _skill_only_slash_items(text), "replace_from": 1},
            )
        ...
        # A bare slash is browsing, so mix frequently used skills into the
        # bounded command list instead of spending the entire cap on registry
        # commands. Argument-stage completers retain their own ordering.
...
# The OpenTUI fork still owns lifecycle/correlation behavior in these
# in-module handlers. Upstream's extracted modules must own every other moved
# handler so fixes made after the extraction are not shadowed by stale copies.
_fork_method_names = frozenset({
    "complete.slash",
    "config.get",
    "delegation.status",
    "model.options",
...
_fork_methods = {
    name: handler for name, handler in _methods.items() if name in _fork_method_names
}
for _m in (
...
_methods.update(_fork_methods)
del _m
del _fork_methods
del _fork_method_names
