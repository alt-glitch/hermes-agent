...
from tui_gateway.agent_runtime_install import (
    apply_service_tier_override as _apply_service_tier_override,
    install_deferred_agent_runtime as _install_deferred_agent_runtime,
)
...
def _attach_built_agent(current: dict, agent) -> None:
    ...
    _install_deferred_agent_runtime(current, agent)
        ...
        try:
            ...
            try:
                agent = _make_agent_with_mcp_registry_fence(
                    sid, key, **_deferred_build_agent_kwargs(current, session_db))
            finally:
...
# admission -> session mutation -> sessions/history. Agent construction and
# tool reconfiguration take only the first lock, so unrelated sessions remain
# responsive while an agent snapshots the registry.
...
def _try_mcp_turn_admission(session: dict, *, sid: str | None = None):
    """Try to claim a prompt against MCP reload without blocking the RPC reader.

    The caller performs the history/running transition while this context is
    held.  Keeping history ownership at the call site avoids recursively taking
    the non-reentrant per-session history lock after the upstream module split.
    """
    ...
    try:
        mutation_context = (
            _session_mutation_lock(session) if sid is not None else contextlib.nullcontext()
        )
        with mutation_context:
            if sid is not None and (
                not _session_registry_matches(sid, session) or session.get("_finalized")
            ):
                yield False
                return
            yield True
