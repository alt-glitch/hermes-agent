...
def _(rid, params: dict) -> dict:
    ...
    def _busy_session_id() -> str | None:
        with _sessions_lock:
            return next((sid for sid, live in _sessions.items() if live.get("running")), None)
        ...
        with _mcp_registry_transition_lock, _mcp_reload_admission_lock:
            ...
            try:
                ack = _get_compute_host_supervisor().reload_mcp(
                    requested_sid, request_id=f"reload-mcp-{rid}")
            except Exception as exc:
    ...
    def _do_full_reload() -> None:
        ...
        with _mcp_registry_transition_lock, _mcp_reload_admission_lock:
            if _busy_session_id() is not None:
                raise _ReloadBusy
            # The launch profile is a profile too: bind its secret scope before
            # reconnecting process-global MCP servers, then restore secondary overlays.
            with _session_profile_runtime_scope({"profile_home": None}):
                ...
                for _ in range(_MCP_RELOAD_MAX_PASSES):
                    ...
                    loaded = after
            with _sessions_lock:
                homes = {sess.get("profile_home") for sess in _sessions.values()
            ...
            for home in sorted(homes - {None}):
                try:
                    ...
                    logger.warning("MCP rediscovery failed for profile %s: %s", home, exc)
...
def _serialize_mcp_registry_transition(fn):
    """Fence the complete tools transaction, including input validation."""
...
def _configure_session_tools(rid, params: dict, sid: str, session) -> dict:
    ...
    mutation_guard = (
        ...
        if original_session is not None else contextlib.nullcontext())
    with mutation_guard:
        with _sessions_lock:
            session = (
                original_session if original_session is not None
                and _sessions.get(sid) is original_session else None)
        history_lock = None
        if session is not None:
            ...
            with history_lock:
                ...
                if session.get("_tools_configuring"):
                    return _err(rid, 4009, "session tools are already being reconfigured")
                session["_tools_configuring"] = True
        try:
            hc = _tools_mod("hermes_cli.config")
            tc = _tools_mod("hermes_cli.tools_config")
            cfg = hc.load_config()
            valid_toolsets = {
                ts_key for ts_key, _, _ in tc.CONFIGURABLE_TOOLSETS
            } | tc._get_plugin_toolset_keys()
            mcp_targets = [name for name in targets if ":" in name]
            unknown = [
                name for name in targets
                if ":" not in name and name not in valid_toolsets]
            toolset_targets = [
                name for name in targets
                if ":" not in name and name in valid_toolsets]
            if toolset_targets:
                tc._apply_toolset_change(cfg, "cli", toolset_targets, action)
            missing_servers = (
                tc._apply_mcp_change(cfg, mcp_targets, action) if mcp_targets else set())
            hc.save_config(cfg)
            info = _reset_session_agent(sid, session) if session else None
            enabled = sorted(tc._get_platform_tools(
                hc.load_config(), "cli", include_default_mcp_servers=False))
            changed = [
                name for name in targets
            ...
            return _ok(rid, {
                ...
                "missing_servers": sorted(missing_servers), "reset": bool(session),
                "unknown": unknown})
        except Exception as exc:
