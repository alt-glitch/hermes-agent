...
class ApprovalResolvedPayload(Payload):
    """``server._emit_approval_lifecycle`` — terminal state of one approval request."""

    request_id: str
    status: str


event("approval.resolved", ApprovalResolvedPayload,
      doc="Terminal state of one approval request (request-scoped, not a client answer).")


class DashboardNewSessionRequestedPayload(Payload):
    """``methods_session.dashboard.new_session_requested`` — a hosted TUI asked the dashboard
    for a fresh chat (dashboard owns the embedded PTY lifecycle)."""

    reason: str = ""


event("dashboard.new_session_requested", DashboardNewSessionRequestedPayload,
      doc="A hosted TUI asked the dashboard for a fresh chat.")
...
__all__ = [
    "ApprovalResolvedPayload", "BillingBlock", "BillingStepUpVerificationPayload",
